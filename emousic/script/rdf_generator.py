class rdfGenerator:
    ...
    def rdf(self, pleasantness, attention, sensitivity, aptitude, moodtag_1, moodtag_2, polarity, battuta_iniziale=None, battuta_finale=None):
        ...
        rdf_string = '  <rdf:Description rdf:about="http://senticnote.net/midi/'+self.song_name+battuta+'">\n'\
                     ...
                     '          <pleasantness xmlns="http://senticnote.net" rdf:datatype="http://w3.org/2001/XMLSchema#float">'+str(pleasantness)+'</pleasantness>\n' \
                     '          <attention xmlns="http://senticnote.net" rdf:datatype="http://w3.org/2001/XMLSchema#float">'+str(attention)+'</attention>\n' \
                     '          <sensitivity xmlns="http://senticnote.net" rdf:datatype="http://w3.org/2001/XMLSchema#float">'+str(sensitivity)+'</sensitivity>\n' \
                     '          <aptitude xmlns="http://senticnote.net" rdf:datatype="http://w3.org/2001/XMLSchema#float">'+str(aptitude)+'</aptitude>\n' \
                     '      </sentics>\n' \
                     ...
                     '          <intensity xmlns="http://senticnote.net" rdf:datatype="http://w3.org/2001/XMLSchema#float">'+str(polarity)+'</intensity>\n' \
                     '      </polarity>\n' \
                     '  </rdf:Description>\n'
        self.file.write(rdf_string)
...
#
# rdf = rdfGenerator("angela")
# rdf.rdf_init()
# rdf.rdf(0,0.2,0,-0.5,'#joy', '#disgust',0.56, 1)
# rdf.rdf(0,0.2,0,-0.5,'#joy', '#disgust',0.56, 2)
# rdf.rdf(0,0.2,0,-0.5,'#joy', '#disgust',0.56) #senza il numero della battuta, lo stesso metodo viene usato per descrivere l'intero brano
# rdf.rdf_conclusion()
